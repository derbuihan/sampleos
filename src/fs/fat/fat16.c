...
#define SAMPLEOS_FAT16_BAD_SECTOR 0xFF7
#define SAMPLEOS_FAT16_UNUSED 0x00
...
int fat16_get_total_items_for_directory(struct disk* disk,
  ...
  if (diskstreamer_seek(stream, directory_start_pos) != SAMPLEOS_ALL_OK) {
    res = -EIO;
    goto out;
  }

  while (1) {
    if (diskstreamer_read(stream, &item, sizeof(item)) != SAMPLEOS_ALL_OK) {
      res = -EIO;
      goto out;
    }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private,
  ...
  if (diskstreamer_read(stream, dir, root_dir_size) != SAMPLEOS_ALL_OK) {
    res = -EIO;
    goto out;
  }
...
int fat16_resolve(struct disk* disk) {
  ...
  if (diskstreamer_read(stream, &fat_private->header,
                        sizeof(fat_private->header)) != SAMPLEOS_ALL_OK) {
    res = -EIO;
    goto out;
  }
  ...
  if (fat16_get_root_directory(
          disk, fat_private, &fat_private->root_directory) != SAMPLEOS_ALL_OK) {
    res = -EIO;
    goto out;
  }
...
static uint32_t fat16_get_first_cluster(struct fat_directory_item* item) {
  return item->low_16_bits_first_cluster |
         (item->high_16_bits_first_cluster << 16);
}
...
static int fat16_get_fat_entry(struct disk* disk, int cluster) {
  ...
  res = diskstreamer_seek(
      stream, fat_table_position * (cluster * SAMPLEOS_FAT16_FAT_ENTRY_SIZE));
  if (res < 0) {
    goto out;
  }
  uint16_t result = 0;
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster,
  ...
  for (int i = 0; i < clusters_ahead; i++) {
    ...
    if (entry == SAMPLEOS_FAT16_BAD_SECTOR) {
      res = -EIO;
      goto out;
    }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk,
  ...
  char tmp_filename[SAMPLEOS_MAX_PATH];
